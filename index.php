...
<body class="font-sans flex flex-col min-h-screen bg-gray-100 text-gray-800 opacity-0 transition-opacity duration-2500">
  ...
  <!-- BANNER -->
  <header class="hero relative text-white text-center rounded-b-xl shadow-lg overflow-hidden
                 p-6 sm:p-12 md:p-16 lg:p-20">
    <h1 class="text-3xl sm:text-4xl md:text-5xl lg:text-6xl mb-2 font-bold">Bem-vindo à MB Esportes</h1>
    <p class="text-base sm:text-lg md:text-xl lg:text-2xl">Confira nossos melhores produtos esportivos!</p>
  </header>
  ...
  <main class="flex-1">
    <section class="max-w-[1200px] mx-auto px-4 py-10">
      ...
      <div class="max-w-[1200px] mx-0 mt-4 mb-8 justify-center items-center">
        <form method="GET" action="index.php" class="flex flex-wrap justify-center items-center gap-2">
          ...
          <input type="text" name="busca" placeholder="Buscar produtos..."
            ...
            class="border border-gray-300 rounded-lg px-4 py-2 w-full sm:w-48 md:w-64 lg:w-80 m-2 outline-none">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 justify-center">
        ...
        // ==== Filtros e query ====
        $where = [];
        if (!empty($_GET['busca'])) $where[] = "(p.nome LIKE '%" . mysqli_real_escape_string($conexao,$_GET['busca']) . "%' OR p.descricao LIKE '%" . mysqli_real_escape_string($conexao,$_GET['busca']) . "%')";
        if (!empty($_GET['categoria'])) $where[] = "p.categoria_id = " . intval($_GET['categoria']);
        if (!empty($_GET['time'])) $where[] = "p.time_id = " . intval($_GET['time']);
        if (!empty($_GET['qualidade'])) $where[] = "p.qualidade_id = " . intval($_GET['qualidade']);
        ...
        if ($result && mysqli_num_rows($result) > 0) {
          while ($produto = mysqli_fetch_assoc($result)) {
            ...
            if (isset($_SESSION['id_usuario'])) {
              ...
              $res_fav = mysqli_query($conexao, "SELECT 1 FROM favoritos WHERE id_usuario=$id_usuario AND id_produto=$id_produto LIMIT 1");
              $favoritado = ($res_fav && mysqli_num_rows($res_fav) > 0);
            }
        ?>
            <div class="relative bg-white border border-gray-300 p-5 rounded-lg text-center shadow-lg transform transition-transform duration-500 hover:scale-105 sm:hover:scale-110">
              <a href="/mbesportes/pages/view_card.php?id=<?= $produto['id'] ?>">
                <?php
                $defaultFallback = '/mbesportes/assets/imgs/bola.png';
                $imgSrc = !empty($produto['imagem']) ? (preg_match('#^https?://#i', trim($produto['imagem'])) ? htmlspecialchars($produto['imagem']) : '/mbesportes/assets/imgs/produtos/' . htmlspecialchars($produto['imagem'])) : $defaultFallback;
                ?>
                ...
                <p class="text-sm text-gray-600 mb-2">Tamanhos disponíveis: <?= htmlspecialchars($produto['tamanhos'] ?? '—') ?></p>
                <p class="text-sm text-gray-600 mb-2">Qualidade: <?= htmlspecialchars($produto['qualidade']) ?></p>
              </a>
              <button class="btn-favorito absolute top-2 right-2 w-10 h-10 rounded-full flex items-center justify-center shadow-lg transition-transform duration-300 hover:scale-125 active:scale-95 border-2 border-gray-300 cursor-pointer <?= $favoritado ? 'favoritado bg-white' : 'bg-white' ?>" data-produto-id="<?= $produto['id'] ?>">
